...
fn chat_rate_without_burst_is_rejected() {
    ...
    config.limits.chat_per_sec = 1;
    config.limits.chat_burst = 0;
...
fn flare_rate_without_burst_is_rejected() {
    let mut config = FileConfig::default();
    config.limits.flare_per_sec = 1;
    config.limits.flare_burst = 0;
    assert!(config.validate().is_err());
}
...
fn disabled_rate_needs_no_burst() {
    let mut config = FileConfig::default();
    config.limits.chat_per_sec = 0;
    config.limits.chat_burst = 0;
    config.limits.flare_per_sec = 0;
    config.limits.flare_burst = 0;
...
fn flood_limits_reach_the_game_config() {
    let mut config = FileConfig::default();
    config.limits.chat_per_sec = 7;
    config.limits.chat_burst = 8;
    config.limits.chat_max_chars = 9;
    config.limits.flare_per_sec = 10;
    config.limits.flare_burst = 11;
    config.limits.commands_per_turn = 12;
    config.limits.command_bytes_per_turn = 13;
    config.pause.min_charge_secs = 14;
    config.limits.flood_kick_multiple = 15;
    let game = config.server_config(false);
...
fn generated_config_carries_the_flood_defaults() {
    ...
    let game = parsed.server_config(false);
