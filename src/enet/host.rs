...
use crate::enet::{Box, Vec, enet_time_get};

use crate::enet::{
    Compressor, ENET_EVENT_TYPE_CONNECT, ENET_EVENT_TYPE_DISCONNECT, ENET_EVENT_TYPE_RECEIVE,
    ENetEvent, ENetHost, ENetPeer, Event, Packet, Peer, PeerID, PeerState, Socket,
    consts::{
...
impl<S: Socket> Host<S> {
    ...
    // The signature predates the elided-lifetime style rule; the body stays
    // byte-identical to the fork so it can still be diffed against it.
    ...
    /// [`Socket::send`].
    // Same newer-compiler style rule as check_events above.
    #[allow(mismatched_lifetime_syntaxes)]
    ...
    /// [`HostSettings::time`](`crate::enet::HostSettings::time`).
    #[must_use]
