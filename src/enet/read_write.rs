use crate::enet::{SocketError, VecDeque};

use crate::enet::{Address, MTU_MAX, PacketReceived, Socket, SocketOptions, Vec};

/// Provides a Read/Write interface for use with [`Host`](`crate::enet::Host`).
///
...
/// [`Host::new`](`crate::enet::Host::new`) will not fail with
/// [`HostNewError::FailedToInitializeSocket`](`crate::enet::error::HostNewError::FailedToInitializeSocket`).
///
/// ```
/// use std::convert::Infallible;
///
/// use veredus::enet::{Host, HostSettings, ReadWrite};
///
...
impl<A: Address, E: SocketError> ReadWrite<A, E> {
    /// Create an intermediate Read/Write socket for use with [`Host`](`crate::enet::Host`).
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    }
