...
//! [`Socket`](`crate::enet::Socket`)) and how connections are made (see
...
#[derive(Clone)]
struct Address<A: crate::enet::Address> {
    address: A,
    id: ConnectionID,
    generation: usize,
}

impl<A: crate::enet::Address> Address<A> {
...
impl<A: crate::enet::Address> crate::enet::Address for Address<A> {
...
/// A trait for implementing connection based sockets, similar to [`Socket`](`crate::enet::Socket`).
///
/// An implementation for [`std::net::TcpStream`] is provided out of the box.
///
/// An implementation of [`ReadWrite`](`crate::enet::ReadWrite`) is provided with an address type of `()`.
#[allow(clippy::missing_errors_doc)]
pub trait Connection {
    /// The address type to use, which must implement [`Address`](`crate::enet::Address`).
    type Address: crate::enet::Address;
    /// Errors returned by this connection.
    type Error: crate::enet::SocketError;
    ...
    /// Try to receive data from the socket into a buffer of size [`MTU_MAX`](`crate::enet::MTU_MAX`).
    ///
    /// A received packet should be written into the provided buffer. If a packet is received that
    /// is larger than [`MTU_MAX`](`crate::enet::MTU_MAX`), it should simply be discarded. ENet will never
    ...
    /// partial. See [`PacketReceived`](`crate::enet::PacketReceived`) for more info.
    fn receive(
        &mut self,
        buffer: &mut [u8; crate::enet::MTU_MAX],
    ) -> Result<Option<crate::enet::PacketReceived>, Self::Error>;
}

impl<E: crate::enet::SocketError> Connection for crate::enet::ReadWrite<(), E> {
    type Address = ();
    type Error = E;
    ...
    fn send(&mut self, buffer: &[u8]) -> Result<usize, Self::Error> {
        crate::enet::Socket::send(self, (), buffer)
    }
    ...
    fn receive(
        ...
        Ok(crate::enet::Socket::receive(self, buffer)?.map(|(_, received)| received))
    }
}

type SocketInterface<C> =
    crate::enet::ReadWrite<Address<<C as Connection>::Address>, <C as Connection>::Error>;
...
///
/// The initiator provides details for [`Host::connect`](`crate::enet::Host::connect`) to start the
...
/// [`Host::connect`](`crate::enet::Host::connect`).
...
impl PeerSettings {
    fn apply<C: Connection + 'static>(&self, peer: &mut crate::enet::Peer<SocketInterface<C>>) {
...
enum PeerState<C: Connection + 'static> {
    Disconnected {
        last_peer_ptr: Option<*mut crate::enet::Peer<SocketInterface<C>>>,
    },
    AwaitingPeer {
        connection: C,
    ...
    HasPeer {
        connection: C,
        address: Address<C::Address>,
        peer_ptr: *mut crate::enet::Peer<SocketInterface<C>>,
    },
    Disconnecting {
        connection: C,
        address: Address<C::Address>,
...
impl<C: Connection> Peer<C> {
    fn peer(&self) -> Option<&mut crate::enet::Peer<SocketInterface<C>>> {
        match self.state {
    ...
    fn peer_or_last_peer(&self) -> Option<&mut crate::enet::Peer<SocketInterface<C>>> {
        match self.state {
    ...
    /// See [`Peer::ping`](`crate::enet::Peer::ping`).
    ...
    /// See [`Peer::send`](`crate::enet::Peer::ping`).
    ///
    /// # Errors
    ///
    /// May return any of the [`error::PeerSendError`](`crate::enet::error::PeerSendError`)) variants on
    /// failure.
    pub fn send(
        &mut self,
        channel_id: u8,
        packet: &crate::enet::Packet,
    ) -> Result<(), crate::enet::error::PeerSendError> {
        self.peer().map_or(
            Err(crate::enet::error::PeerSendError::NotConnected),
            |peer| peer.send(channel_id, packet),
        )
    }

    /// See [`Peer::disconnect`](`crate::enet::Peer::disconnect`).
    ...
    /// See [`Peer::disconnect_now`](`crate::enet::Peer::disconnect_now`).
    ...
    /// See [`Peer::disconnect_later`](`crate::enet::Peer::disconnect_later`).
    ...
    /// See [`Peer::reset`](`crate::enet::Peer::reset`).
    ...
    /// See [`Peer::set_timeout`](`crate::enet::Peer::set_timeout`).
    ...
    /// See [`Peer::set_ping_interval`](`crate::enet::Peer::set_ping_interval`).
    ...
    /// See [`Peer::set_throttle`](`crate::enet::Peer::set_throttle`).
    ...
    /// See [`Peer::mtu`](`crate::enet::Peer::mtu`).
    #[must_use]
    pub fn mtu(&self) -> u16 {
        self.peer_or_last_peer().map_or(0, |peer| peer.mtu())
    }

    /// See [`Peer::set_mtu`](`crate::enet::Peer::set_mtu`).
    ///
    /// # Errors
    ///
    /// Returns [`error::BadParameter`](`crate::enet::error::BadParameter`) if `mtu` is greater than
    /// [`consts::PROTOCOL_MAXIMUM_MTU`](`crate::enet::consts::PROTOCOL_MAXIMUM_MTU`) or less than
    /// [`consts::PROTOCOL_MINIMUM_MTU`](`crate::enet::consts::PROTOCOL_MINIMUM_MTU`).
    pub fn set_mtu(&mut self, mtu: u16) -> Result<(), crate::enet::error::BadParameter> {
        if mtu > crate::enet::consts::PROTOCOL_MAXIMUM_MTU as u16
            || mtu < crate::enet::consts::PROTOCOL_MINIMUM_MTU as u16
        {
            return Err(crate::enet::error::BadParameter {
    ...
    /// See [`Peer::state`](`crate::enet::Peer::state`).
    #[must_use]
    pub fn state(&self) -> crate::enet::PeerState {
        self.peer_or_last_peer()
            .map_or(crate::enet::PeerState::Disconnected, |peer| peer.state())
    }

    /// See [`Peer::connected`](`crate::enet::Peer::connected`).
    #[must_use]
    pub fn connected(&self) -> bool {
        self.state() == crate::enet::PeerState::Connected
    }

    /// See [`Peer::channel_count`](`crate::enet::Peer::channel_count`).
    #[must_use]
    ...
    /// See [`Peer::incoming_bandwidth`](`crate::enet::Peer::incoming_bandwidth`).
    #[must_use]
    ...
    /// See [`Peer::outgoing_bandwidth`](`crate::enet::Peer::outgoing_bandwidth`).
    #[must_use]
    ...
    /// See [`Peer::incoming_data_total`](`crate::enet::Peer::incoming_data_total`).
    #[must_use]
    ...
    /// See [`Peer::outgoing_data_total`](`crate::enet::Peer::outgoing_data_total`).
    #[must_use]
    ...
    /// See [`Peer::outgoing_queue_bytes`](`crate::enet::Peer::outgoing_queue_bytes`).
    #[must_use]
    ...
    /// See [`Peer::packets_sent`](`crate::enet::Peer::packets_sent`).
    #[must_use]
    ...
    /// See [`Peer::packets_lost`](`crate::enet::Peer::packets_lost`).
    #[must_use]
    ...
    /// See [`Peer::packet_loss`](`crate::enet::Peer::packet_loss`).
    #[must_use]
    ...
    /// See [`Peer::packet_loss_variance`](`crate::enet::Peer::packet_loss_variance`).
    #[must_use]
    ...
    /// See [`Peer::ping_interval`](`crate::enet::Peer::ping_interval`).
    #[must_use]
    ...
    /// See [`Peer::round_trip_time`](`crate::enet::Peer::round_trip_time`).
    #[must_use]
    ...
    /// See [`Peer::round_trip_time_variance`](`crate::enet::Peer::round_trip_time_variance`).
    #[must_use]
    ...
    /// See [`Peer::address`](`crate::enet::Peer::address`).
    #[must_use]
...
/// An ENet event, like [`Event`](`crate::enet::Event`), but without peer references.
///
...
impl<C: Connection> Host<C> {
    ...
    /// # Errors
    ///
    /// Returns [`error::BadParameter`](`crate::enet::error::BadParameter`) if one of the host settings
    /// was invalid:
    /// - If [`HostSettings::channel_limit`](`crate::enet::HostSettings::channel_limit`) is equal to `0`.
    /// - If [`HostSettings::incoming_bandwidth_limit`](`crate::enet::HostSettings::incoming_bandwidth_limit`)
    ///   is equal to `Some(0)`.
    /// - If [`HostSettings::outgoing_bandwidth_limit`](`crate::enet::HostSettings::outgoing_bandwidth_limit`)
    ///   is equal to `Some(0)`.
    /// - If [`HostSettings::peer_limit`](`crate::enet::HostSettings::peer_limit`) is equal to `0` or
    ///   greater than [`consts::PROTOCOL_MAXIMUM_PEER_ID`](`crate::enet::consts::PROTOCOL_MAXIMUM_PEER_ID`).
    pub fn new(
        settings: crate::enet::HostSettings,
    ) -> Result<Self, crate::enet::error::BadParameter> {
        ...
        Ok(Self {
            host: crate::enet::Host::new(crate::enet::ReadWrite::new(), settings).map_err(
                |err| match err {
                    crate::enet::error::HostNewError::BadParameter(err) => err,
                    crate::enet::error::HostNewError::FailedToInitializeSocket(..) => {
                        unreachable!()
                    }
                },
            )?,
    ...
    fn handle_event(&mut self, event: crate::enet::EventNoRef) -> Event<C> {
        let now = self.host.now();
        match event {
            crate::enet::EventNoRef::Connect { peer, data } => {
            ...
            crate::enet::EventNoRef::Disconnect { peer, data } => {
                let peer = self.host.peer_mut(peer);
        ...
        for peer in &mut self.peers {
            ...
            match &mut peer.state {
                ...
                | PeerState::Disconnecting {
                    ...
                    let mut buffer = [0; crate::enet::MTU_MAX];
                    match connection.receive(&mut buffer) {
                        Ok(Some(crate::enet::PacketReceived::Complete(size))) => {
                            self.host
                                .socket_mut()
    ...
    /// See [`Host::flush`](`crate::enet::Host::flush`).
    pub fn flush(&mut self) {
        self.host.flush();
    }

    /// See [`Host::peer_limit`](`crate::enet::Host::peer_limit`).
    #[must_use]
    ...
    /// See [`Host::broadcast`](`crate::enet::Host::broadcast`).
    pub fn broadcast(&mut self, channel_id: u8, packet: &crate::enet::Packet) {
        self.host.broadcast(channel_id, packet);
    }

    /// See [`Host::channel_limit`](`crate::enet::Host::channel_limit`).
    #[must_use]
    pub fn channel_limit(&self) -> usize {
        self.host.channel_limit()
    }

    /// See [`Host::set_channel_limit`](`crate::enet::Host::set_channel_limit`).
    ///
    /// # Errors
    ///
    /// Returns [`error::BadParameter`](`crate::enet::error::BadParameter`) if `channel_limit` is `0`.
    pub fn set_channel_limit(
        &mut self,
        channel_limit: usize,
    ) -> Result<(), crate::enet::error::BadParameter> {
        self.host.set_channel_limit(channel_limit)
    }

    /// See [`Host::bandwidth_limit`](`crate::enet::Host::bandwidth_limit`).
    #[must_use]
    pub fn bandwidth_limit(&self) -> (Option<u32>, Option<u32>) {
        self.host.bandwidth_limit()
    }

    /// See [`Host::set_bandwidth_limit`](`crate::enet::Host::set_bandwidth_limit`).
    ///
    /// # Errors
    ///
    /// Returns [`error::BadParameter`](`crate::enet::error::BadParameter`) if `incoming_bandwidth_limit`
    ...
    /// See [`Host::mtu`](`crate::enet::Host::mtu`).
    #[must_use]
    pub fn mtu(&self) -> u16 {
        self.host.mtu()
    }

    /// See [`Host::set_mtu`](`crate::enet::Host::set_mtu`).
    ///
    /// # Errors
    ///
    /// Returns [`error::BadParameter`](`crate::enet::error::BadParameter`) if `mtu` is greater than
    ...
    /// See [`Host::now`](`crate::enet::Host::now`).
    #[must_use]
...
impl Connection for TcpStream {
    ...
    fn receive(
        ...
        match self.read(buffer) {
            Ok(recv_length) => Ok(Some(crate::enet::PacketReceived::Complete(recv_length))),
